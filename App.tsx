...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => { p5Obj.remove(); p5Instance.current = null; };
  }, []);
  ...
  const handlePresetChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    (Object.keys(p) as Array<keyof Preset>).forEach(key => { if (key !== 'name' && key in newParams) { // @ts-ignore
        newParams[key] = p[key]; } });
    setParams(newParams);
  ...
  const toggleFullscreen = () => {
    if (!document.fullscreenElement) { document.getElementById('canvasContainer')?.requestFullscreen().catch(() => displayToast('全画面表示エラー'));
